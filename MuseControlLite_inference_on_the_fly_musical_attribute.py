...
from MuseControlLite_train_musical_attributes import dynamics_extractor, rhythm_extractor, melody_extractor,  evaluate_f1_rhythm
...
def main(config):
    ...
    output_dir = config["output_dir"] + f"text_{config['guidance_scale_text']}_con_{config['guidance_scale_con']}_audio_{config['guidance_scale_audio']}_{'_'.join(config['condition_type'])}"
    ...
    with torch.no_grad():
        for i, audio_file in enumerate(dataset):
            ...
            if "dynamics" in config["condition_type"]:
                ...
                # roll_torch = random.choice(dynamic_roll)
                # dynamics_condition = roll_torch.cuda()
                ...
                masked_extracted_dynamics_condition = torch.full_like(extracted_dynamics_condition, fill_value=0)
                ...
                # extracted_dynamics_condition[: ,:, : mask_start] = 0
                # extracted_dynamics_condition[: ,:, mask_end:] = 0
            else: 
                extracted_dynamics_condition = torch.full((1, 192, 1024), 0, device='cuda')
            ...
            if "rhythm" in config["condition_type"]:
                ...
                masked_extracted_rhythm_condition = torch.full_like(extracted_rhythm_condition, fill_value=0)
                ...
                # extracted_rhythm_condition[: ,:, : mask_start] = 0
                # extracted_rhythm_condition[: ,:, mask_end:] = 0           
            else: 
                extracted_rhythm_condition = torch.full((1, 192, 1024), 0, device='cuda')
            ...
            if "melody" in config["condition_type"]:
                ...
                masked_extracted_melody_condition = torch.full_like(extracted_melody_condition, fill_value=0)
                ...
                extracted_melody_condition = torch.full((1, 192, 1024), 0, device='cuda')
            ...
            audio = audio.to(memory_format=torch.contiguous_format).float()
            audio_condition = pipe.vae.encode(audio.unsqueeze(0).to(dtype=weight_dtype).cuda()).latent_dist.sample()
            audio = pipe.vae.decode(audio_condition).sample.squeeze(2)
            extracted_audio_condition = audio_condition.repeat_interleave(desired_repeats, dim=1)
            masked_extracted_audio_condition = torch.full_like(extracted_audio_condition, 0)

            extracted_condition = torch.concat((extracted_rhythm_condition, extracted_dynamics_condition, extracted_melody_condition, extracted_audio_condition), dim=1).to(weight_dtype)
            masked_extracted_condition = torch.concat((masked_extracted_rhythm_condition, masked_extracted_dynamics_condition, masked_extracted_melody_condition, masked_extracted_audio_condition), dim=1).to(weight_dtype)
            ...
            input_probabilities = processor(audio_file)[:3000]
            generated_probabilities = processor(gen_file_path)[:3000]
            ...
            ax = axes[0, 2]
            ax.plot(input_time_axis, input_probabilities[:3000, 0],
                    label="Input Beat Probability")
            ax.plot(input_time_axis, input_probabilities[:3000, 1],
            ...
            ax = axes[1, 2]
            ax.plot(generate_time_axis, generated_probabilities[:3000, 0],
                    color='orange', label="Generated Beat Probability")
            ax.plot(generate_time_axis, generated_probabilities[:3000, 1],
