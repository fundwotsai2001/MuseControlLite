...
class StableAudioAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
        # print(attn.to_k.weight.shape)  # Shape of the weights
...
class StableAudioAttnProcessor2_0_rotary(torch.nn.Module):
    ...
    used in the Stable Audio model. It applies rotary embedding on query and key vector, and allows MHA, GQA or MQA.
    """
    ###### add rotary to Q,K,V, slice out global hidden state for layer 0 ###### 
    ...
    def __call__(
        ...
        #     print("hidden_states_sliced dtype:", hidden_states_sliced.dtype)
        #     query_sliced = attn.to_q(hidden_states_sliced)
        #     query_sliced = query_sliced.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)
        #     query = query_sliced
        #######################################################
        # print("ip_hidden_states", ip_hidden_states.device)
        # device = next(self.to_k_ip.parameters()).device
        # print("self.to_k_ip", device)
...
class StableAudioAttnProcessor2_0_rotary_double(torch.nn.Module):
    ...
    def __call__(
        ...
        position_ids_value = torch.arange(ip_value_length, dtype=torch.long, device=value.device)
        position_ids_value = position_ids_value.unsqueeze(0).expand(batch_size, -1)  # Shape: [batch_size, seq_len_key]
        # print("position_ids_value", position_ids_value)
        # print("position_ids_key", position_ids_key)
        # print("position_ids_query", position_ids_query)
        ...
        # print("position_ids_value_audio", position_ids_value_audio)
        # print("position_ids_key_audio", position_ids_key_audio)
        # print("position_ids_query_audio", position_ids_query_audio)
        cos, sin = self.rotary_emb(query, position_ids_query)
