...
class StableAudioAttnProcessor2_0_rotary_scale_up(torch.nn.Module):
    ...
    def __init__(self, layer_id, hidden_size, name, cross_attention_dim=None, num_tokens=4, scale=1.0):
        ...
        self.num_tokens = num_tokens
        self.scale = scale
        self.to_k_ip = nn.Linear(cross_attention_dim or hidden_size, 1536, bias=False)
        self.to_v_ip = nn.Linear(cross_attention_dim or hidden_size, 1536, bias=False)
        self.name = name
        self.conv_out = zero_module(nn.Conv1d(1536,1536,kernel_size=1, padding=0, bias=False))     
        self.rotary_emb = LlamaRotaryEmbedding(dim = 64)
        self.to_k_ip.weight.requires_grad = True
...
class StableAudioAttnProcessor2_0_rotary_Wq(torch.nn.Module):
    ...
    def __call__(
        ...
        return hidden_states
class StableAudioAttnProcessor2_0_rotary_smallest(torch.nn.Module):
    r"""
    Processor for implementing scaled dot-product attention (enabled by default if you're using PyTorch 2.0). This is
    used in the Stable Audio model. It applies rotary embedding on query and key vector, and allows MHA, GQA or MQA.
    """
    def __init__(self, layer_id, hidden_size, name, cross_attention_dim=None, num_tokens=4, scale=1.0):
        ...
        self.num_tokens = num_tokens
        self.scale = scale
        self.to_k_ip = nn.Linear(128, hidden_size, bias=False)
        self.to_v_ip = nn.Linear(128, hidden_size, bias=False)
        self.name = name
        # self.conv_out = zero_module(nn.Conv1d(1536,1536,kernel_size=1, padding=0, bias=False))     
        self.rotary_emb = LlamaRotaryEmbedding(dim = 64)
        self.to_k_ip.weight.requires_grad = True
    ...
    def __call__(
        ...
        print("ip_key.shape", ip_key.shape)
        ip_hidden_states = F.scaled_dot_product_attention(
                query_pos, ip_key, ip_value, attn_mask=None, dropout_p=0.0, is_causal=False
            )
        ip_hidden_states = ip_hidden_states.transpose(1, 2).reshape(batch_size, -1, attn.heads * head_dim)
        ...
        # print("ip_hidden_states.shape", ip_hidden_states.shape)
        # Combine the output of the two cross-attention layers
        hidden_states = hidden_states + self.scale * ip_hidden_states
        # linear proj
        hidden_states = attn.to_out[0](hidden_states)
