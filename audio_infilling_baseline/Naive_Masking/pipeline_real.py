...
class AudioInfillingPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-audio generation (with audio infilling) using StableAudio.
    ...
    def __call__(
        ...
        Args:
            ...
            reference_duration (`float`, *optional*, defaults to 15.0):
                Duration of reference used to perform audio infilling.
        ...
        # create mask for ref and gen
        reference_latents = self.vae.encode(initial_audio_waveforms).latent_dist.sample(generator)
        reference_noise = torch.randn_like(reference_latents)
        M_ref = torch.zeros_like(latents)
        print("M_ref", M_ref.shape)
        M_ref[:, :, :int(705600/2048)] = 1
        M_ref[:, :, int(1411200/2048):] = 1
        M_gen = 1 - M_ref
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # perform naive masking
                timestep = torch.tensor([t])
                reference = torch.zeros_like(latents)
                noisy_reference = self.scheduler.add_noise(reference_latents, reference_noise, timestep)
                reference[:, :, :int(705600/2048)] = noisy_reference[:, :, :int(705600/2048)]
                reference[:, :, int(1411200/2048):] = noisy_reference[:, :, int(1411200/2048):] 
                latents = M_ref * reference + M_gen * latents
