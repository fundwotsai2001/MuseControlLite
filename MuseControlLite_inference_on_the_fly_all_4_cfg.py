    ...
    if config["apadapter"]:
        ...
        if config["transformer_ckpt"] is not None:
            if "bin" in config["transformer_ckpt"]:
                ...
                audio_state_dict = torch.load(config["transformer_ckpt"])
            elif "safetensors" in config["transformer_ckpt"]:
                state_dict = load_file(config["transformer_ckpt"], device="cuda")
                audio_state_dict = load_file(config["transformer_ckpt"], device="cuda")
        ...
        for i, prompt_texts in enumerate(config['text']):
            if config["apadapter"]:
                ...
                if config['use_audio_mask']:
                    ...
                    print("audio_mask_start:audio_mask_end", audio_mask_start, audio_mask_end)
                elif config['use_musical_attribute_mask']:
                ...
                # Use multiple cfg
                extracted_condition_musical = torch.concat((extracted_rhythm_condition, extracted_dynamics_condition, extracted_melody_condition, extracted_blank_condition), dim=1)
                ...
                extracted_condition_audio = torch.concat((extracted_rhythm_condition, extracted_dynamics_condition, extracted_melody_condition, extracted_audio_condition), dim=1)

                extracted_condition = torch.concat((masked_extracted_condition, masked_extracted_condition, extracted_condition_musical, extracted_condition_audio), dim=0)
