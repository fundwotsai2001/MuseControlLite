...
from MuseControlLite_attn_processor import (
    ...
    StableAudioAttnProcessor2_0_rotary_smallest,
)
...
def log_validation(val_dataloader, condition_type, pipeline, config, weight_dtype, global_step):
    ...
    for step, batch in enumerate(val_dataloader):
        ...
        masked_extracted_melody_condition = torch.full_like(melody_condition.to(torch.float32), fill_value=0)
         
        extracted_melody_condition = F.interpolate(melody_condition, size=1024, mode='linear', align_corners=False)
        print("extracted_melody_condition", extracted_melody_condition.shape)
...
def main():
    ...
    processor_classes = {
        ...
        "smallest": StableAudioAttnProcessor2_0_rotary_smallest,
    }
